...
const showProducts = (products) => {
   ...
   for (const product of allProducts) {
      ...
      <div class="single-product h-100">
         ...
         data-bs-target="#exampleModal" class="btn btn-outline-secondary mb-2 rounded-1 mt-1">Details
         </button>
...
const updatePrice = (id, value) => {
   ...
   const convertPrice = parseFloat(convertedOldPrice);
   const total = convertPrice + value;
...
const updateTaxAndCharge = () => {
   ...
   const x = (priceConverted * 0.2).toFixed(2);

   if (priceConverted > 500) {
      setInnerText('delivery-charge', 60);
      setInnerText('total-tax', x);
   }
